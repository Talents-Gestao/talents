                    ...
                    <tr>
                        <td style="padding: 16px 32px 32px; text-align: left;">
                            ...
                            @if ($user->hasCompletedRegistration())
                                @if ($company)
                                    <p style="margin: 0 0 16px;">Foi solicitada a <strong>redefinição de senha</strong> do seu acesso ao <strong>portal Talents</strong> para a empresa <strong>{{ $company->name }}</strong>.</p>
                                @else
                                    <p style="margin: 0 0 16px;">Foi solicitada a <strong>redefinição de senha</strong> do seu acesso à <strong>equipe administrativa</strong> da plataforma Talents.</p>
                                @endif
                            ...
                            @else
                                @if ($company)
                                    <p style="margin: 0 0 16px;">Foi criado um <strong>acesso ao portal Talents</strong> para a empresa <strong>{{ $company->name }}</strong>.</p>
                                @else
                                    <p style="margin: 0 0 16px;">Foi criado um <strong>acesso à equipe administrativa</strong> da plataforma Talents.</p>
                                @endif
                                <p style="margin: 0 0 16px;">Seu usuário é o e-mail: <strong>{{ $user->email }}</strong></p>
