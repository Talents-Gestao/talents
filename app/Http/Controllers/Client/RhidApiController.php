...
class RhidApiController extends Controller
{
    ...
    public function espelhoMonthlyCompliance(Request $request, EspelhoScheduleAdherenceService $adherence): JsonResponse
    {
        $company = $this->company($request);
        $v = $request->validate([
            'ini' => ['required', 'date'],
        ...
        return response()->json(
            $adherence->monthlyComplianceRanking($company, $ini, $fim, $request->user()),
        );
    }
